...
class ApartamentoListCreateAPIView(APIView):
    ...
    def get(self, request):
        ...
        if not request.user.has_perm('Apartments.manage_apartamentos'):
...
class ApartamentoDetailAPIView(APIView):
    ...
    def get(self, request, pk):
        ...
        if not request.user.has_perm('Apartments.manage_apartamentos') and apt.propietario != request.user:
            return Response({'detail': 'Solo administradores pueden ver apartamentos.'}, status=status.HTTP_403_FORBIDDEN)
